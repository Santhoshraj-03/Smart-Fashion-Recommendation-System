...
@details.route("/details/<username>/<email>/<mobile>/<sno>/",methods=['POST','GET'])
def open(username,email,mobile,sno):
    name = username
    phone = mobile
    ...
    return render_template("details.html",fullname=username,email=email,mobile=mobile,sno=sno,address=address,pincode=pincode,state=state,city=city)
